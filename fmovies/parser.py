...
class HtmlParser:
        ...
        try:
            ...
            if suggestion:
                soup = soup.find_all(
                    lambda tag: (
                        tag.name == "div"
                        and tag.get("class") == ["tab-content"]
                        and tag.get("data-name") in suggestion
                    )
                )
                return [
                    item for div in soup for item in div.find_all("div", class_="item")
                ]
    ...
    def get_media(url: str, suggestion: list = None) -> dict:
        ...
        if "error" in movie_divs:
            return movie_divs
        return {
            "media": [HtmlParser.create_media_dict_from_div(div) for div in movie_divs]
        }
